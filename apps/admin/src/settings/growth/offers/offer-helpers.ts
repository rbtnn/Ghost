...
const isFreeMonthsRetentionOffer = ({
  amount,
  duration,
}: {
  amount: number;
...
export const generateRetentionOfferName = (
  input: { amount: number; duration: string; durationInMonths: number },
  hash: string,
): string => {
...
export { MAX_RETENTION_OFFER_NAME_LENGTH };
...
export const getOfferDiscount = (
  ...
  let discountOffer = '';
  const originalPrice =
    cadence === 'month' ? (tier?.monthly_price ?? 0) : (tier?.yearly_price ?? 0);
