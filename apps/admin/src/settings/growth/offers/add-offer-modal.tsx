        ...
        <section>
          ...
          <div className="flex flex-col gap-6">
            ...
            <Field data-invalid={Boolean(errors.displayTitle) || undefined}>
              ...
              <Input
                aria-invalid={Boolean(errors.displayTitle) || undefined}
                id="offer-display-title"
        ...
        <section className="mt-4">
          ...
          <div className="flex flex-col gap-6">
            ...
            <Field>
              ...
              >
                <SelectTrigger aria-label="Tier — Cadence" data-testid="tier-cadence-select-offers">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
            ...
            {overrides.type !== 'trial' && (
              <>
                    ...
                    <InputGroupInput
                      ...
                      value={
                        overrides.type === 'fixed'
                          ? overrides.fixedAmount === 0
                            ? ''
                            : overrides.fixedAmount?.toString()
                          : overrides.percentAmount === 0
                            ? ''
                            : overrides.percentAmount?.toString()
                      }
                      onChange={(e) => {
                        handleAmountInput(e);
                      }}
                      ...
                      <Select
                        value={
                          overrides.type === 'percent'
                            ? amountOptions[0].value
                            : amountOptions[1].value
                        }
                        onValueChange={handleAmountTypeChange}
                      >
                        <SelectTrigger
                          aria-label="Amount type"
                          className="h-7 w-20 border-0 bg-transparent px-2 shadow-none focus-visible:ring-0"
                          data-testid="amount-type-select-offers"
                        >
                          <SelectValue />
                        </SelectTrigger>
                ...
                <Field>
                  ...
                  >
                    <SelectTrigger aria-label="Duration" data-testid="duration-select-offers">
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
                    ...
                    <Field data-invalid={Boolean(errors.durationInMonths) || undefined}>
                      <InputGroup data-invalid={Boolean(errors.durationInMonths) || undefined}>
                        <InputGroupInput
                          ...
                          type="number"
                          value={
                            overrides.durationInMonths === 0
                              ? ''
                              : String(overrides.durationInMonths)
                          }
                          onChange={(e) => {
                        ...
                        <InputGroupAddon align="inline-end">
                          <InputGroupText>
                            {overrides.durationInMonths === 1 ? 'month' : 'months'}
                          </InputGroupText>
                        </InputGroupAddon>
                      </InputGroup>
...
const AddOfferModal = () => {
  ...
  const [selectedTier, setSelectedTier] = useState({
    ...
    dataset: {
      ...
      currency: tierCadenceOptions[0]?.value
        ? parseData(tierCadenceOptions[0]?.value).currency
        : '',
    },
  });
  ...
  const { formState, updateForm, handleSave, saveState, okProps, validate, errors, clearError } =
    useForm({
      initialState: {
      ...
      onValidate: () => {
        ...
        if (
          ...
          (formState.percentAmount < 1 || formState.percentAmount > 100)
        ) {
          newErrors.amount = 'Enter an amount between 1 and 100%.';
        }

        if (
          (formState.type === 'fixed' && formState.fixedAmount === 0) ||
          (formState.type === 'fixed' && formState.fixedAmount < 1)
        ) {
          newErrors.amount = 'Enter an amount greater than 0.';
        }
        ...
        if (
          ...
          (!Number.isInteger(formState.durationInMonths) || formState.durationInMonths < 1)
        ) {
  ...
  const iframe = <PortalFrame href={href || ''} portalParent="offers" />;
  return (
    <PreviewModalContent
      backDropClick={false}
      cancelLabel="Cancel"
      ...
      onOk={async () => {
        ...
        if (!isErrorsEmpty) {
          toast.dismiss();
          toast.info("Can't save offer", {
            description: 'Make sure you filled all required fields',
          });
          return;
        }
