...
export const formatPriceBlockAmount = (amount: number, currency: string) => {
  ...
  return value.toLocaleString(
    undefined,
    value % 1 === 0 ? undefined : { minimumFractionDigits: 2, maximumFractionDigits: 2 },
  );
};

const PriceBlock: React.FC<{ sub: MemberSubscription; kind: SubscriptionKind }> = ({
  sub,
  kind,
}) => {
...
const SubscriptionRow: React.FC<{
  ...
  return (
    <div
      className={cn(
        'flex items-start justify-between gap-4 py-5 first:pt-0 last:pb-0',
        showDivider && 'border-t border-border',
      )}
      data-testid="member-subscription"
    >
      <div className="flex min-w-0 flex-1 items-start gap-4">
        <PriceBlock kind={kind} sub={sub} />
        <div className="min-w-0 flex-1 pt-2">
          <div className="flex items-center gap-2">
            <span className="min-w-0 truncate font-semibold" data-testid="member-subscription-tier">
              {sub.tier?.name ?? 'Subscription'}
            </span>
            <Badge
              data-testid="member-subscription-status"
              variant={status === 'Canceled' ? 'secondary' : 'success'}
            >
              {status}
            </Badge>
          </div>
      ...
      {/* Gift subs get no action menu (Ember parity). Comp subs get their
                ...
                Stripe links. */}
      {kind === 'paid' && <MemberSubscriptionActions memberId={member.id} subscription={sub} />}
 ...
 */
const MemberSubscriptionsSection: React.FC<MemberSubscriptionsSectionProps> = ({
  member,
  canAddComp,
}) => {
  ...
  const addCompButton = isAddCompAllowed ? (
    <Button data-testid="add-complimentary" variant="outline" onClick={() => setShowAddComp(true)}>
